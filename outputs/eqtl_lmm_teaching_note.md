...
- `COI_workshop_13th_Aug_Host genetic and epigenetic determinants of variable response to infection.docx`
...
## Suggested talking points for class

- Start with the unit of observation: one row is one sample, but repeated rows can belong to one person.
- Ask why `subject_id` is in the model: it identifies the grouping structure for the random intercept.
- Interpret genotype dosage as 0, 1, or 2 copies of the effect allele.
- Distinguish a genotype association from an interaction: average effect versus effect that changes with infection time.
- Use the violin plot for distributions and the beta/p-value annotation for the fitted average association.
- Use the interaction plot to compare slopes, not only the heights of the coloured lines.
- Emphasise that a small p-value is not the same as a large effect or an important mechanism.
- Check sample sizes and uncertainty, especially for genotype 2 and the 6-hour time point.
- Explain that a real eQTL scan tests many variants and genes, so multiple-testing correction is essential.
- Treat the corrected-expression step as a teaching convenience and compare it with a single model containing the covariates directly.
